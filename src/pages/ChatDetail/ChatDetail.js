...
export default function ChatDetail() {
    ...
    function deleteChat() {
        messages.forEach(message => {
            const deleteFor = message.deleteFor
            deleteFor.push(auth.id)

            if (deleteFor.length > 1) {
                useDb.collection('chats').doc(params.id).collection('messages').doc(message.id).delete()
            } else {
                useDb.collection('chats').doc(params.id).collection('messages').doc(message.id).set({deleteFor}, {merge: true})
            }
        })
        setMenu(false)
    }
    ...
    useEffect(() => {
        ...
        const getMessage = useDb.collection('chats').doc(params.id).collection('messages').orderBy('sendTime').onSnapshot(res => {
            const data = res.docs.map(doc => doc.data()).filter(message => !message.deleteFor.includes(auth.id))
            setMessages(data)
